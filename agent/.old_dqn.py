...
class DQNAgent:
    ...
    def replay(self):
        ...
        for state, action, reward, next_state, terminated, truncated in batch:
            ...
            # Forward pass through individual MLPs
            ...
            # Concatenate outputs from individual MLPs
            combined_value = torch.cat((rgb_value, lidar_value, rest_value), dim=1)  # Corrected dimension here

            # Append the combined value to the states list
            states.append(combined_value)
            targets.append(target)

        states = torch.stack(states)  # Stack the tensors in the states list
        targets = torch.stack(targets).unsqueeze(1)  # Adjust shape to (batch_size, 1)
